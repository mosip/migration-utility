...
import io.mosip.pms.encryptutility.constants.AppConstants;
import io.mosip.pms.encryptutility.constants.ErrorCode;
import io.mosip.pms.encryptutility.dto.CryptoRequestDto;
import io.mosip.pms.encryptutility.dto.CryptoResponseDto;
import io.mosip.pms.encryptutility.dto.RequestWrapper;
...
public class KeyManagerUtil {
    ...
    public String encryptData(String data) {
        try {
            ...
            RequestWrapper<CryptoRequestDto> requestWrapper = new RequestWrapper<>();
            ...
            if (response.containsKey(AppConstants.ERRORS)) {
                List<Map<String, Object>> errorList = (List<Map<String, Object>>) response.get(AppConstants.ERRORS);
                if (errorList != null && !errorList.isEmpty()) {
                    logger.debug("Error occurred while fetching data: {}", errorList);
                    throw new PartnerEncryptUtilityException(String.valueOf(errorList.get(0).get(AppConstants.ERRORCODE)),
                            String.valueOf(errorList.get(0).get(AppConstants.ERRORMESSAGE)));
                }
            }
            if (!response.containsKey(AppConstants.RESPONSE) || response.get(AppConstants.RESPONSE) == null) {
